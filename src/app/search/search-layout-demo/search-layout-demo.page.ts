...
export class SearchLayoutDemoPage implements OnInit{
  ...
  ngOnInit() {
    this.activatedRoute.queryParams.subscribe(
      data => {
        ...
        this.pageNum = typeof data['page'] === 'string' && parseInt(data['page']) - 1 >= 0 ?
                        parseInt(data['page']) - 1 : this.pageNum;
      ...
      this.searchService.featuredSearch({
        ...
        data => {
          if (data._embedded && data._embedded.featuredResult) {
            for (let i = 0; i < data._embedded.featuredResult.length; i++) {
              if (data._embedded.featuredResult[i].parentOrganizationHierarchy) {
                data._embedded.featuredResult[i].parentOrganizationHierarchy.name =
                  new CapitalizePipe()
                      .transform(data._embedded.featuredResult[i].parentOrganizationHierarchy.name
                      .replace(/[_-]/g, ' '));
              }
              if (data._embedded.featuredResult[i].type) {
                data._embedded.featuredResult[i].type =
                  new CapitalizePipe().transform(data._embedded.featuredResult[i].type);
              }
            }
    ...
    this.searchService.runSearch({
      ...
      data => {
        if (data._embedded && data._embedded.results) {
          for (var i = 0; i < data._embedded.results.length; i++) {
            ...
            if (data._embedded.results[i].parentOrganizationHierarchy) {
              if (data._embedded.results[i].parentOrganizationHierarchy.name.indexOf('.') > -1) {
               data._embedded.results[i].parentOrganizationHierarchy.name =
                data._embedded.results[i].parentOrganizationHierarchy.name
                .substring(0, data._embedded.results[i].parentOrganizationHierarchy.name.indexOf('.'));
              }
              data._embedded.results[i].parentOrganizationHierarchy.name =
                new CapitalizePipe()
                .transform(data._embedded.results[i].parentOrganizationHierarchy.name.replace(/[_-]/g, ' '));
            }
          ...
          let maxAllowedPages = data.page['maxAllowedRecords'] / this.showPerPage;
  ...
  pageChange(pagenumber) {
    ...
    this.router.navigate(['/search/demo'], navigationExtras);
  }
