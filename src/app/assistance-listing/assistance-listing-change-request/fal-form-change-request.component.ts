...
export class FALFormChangeRequestComponent implements OnInit {
  ...
  createChangeRequestForm() {
    this.activatedRoute.queryParams.subscribe(params => {
      if (this.validateRequestType(params['type'])) {
        this.requestType = params['type'];
        if ((params['type'] === 'archive_request' && (this.permissions['INITIATE_CANCEL_ARCHIVE_CR'] || this.permissions['APPROVE_REJECT_ARCHIVE_CR'])) ||
          (params['type'] === 'unarchive_request' && (this.permissions['INITIATE_CANCEL_UNARCHIVE_CR'] || this.permissions['APPROVE_REJECT_UNARCHIVE_CR']))) {
          this.pageReady = true;
          this.createForm(params['type']);
  ...
  private prepareChangeRequestData(): ChangeRequestModel {
    ...
    if (this.falChangeRequestForm.valid) {
      if (this.requestType == 'title_request') {
        preparedData.data.title = this.falChangeRequestForm.get('title').value;
      ...
      } else if (this.requestType == 'number_request') {
        preparedData.data.programNumber = this.falChangeRequestForm.get('programNumber').value;
      }
    }
