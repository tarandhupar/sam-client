...
describe('src/app/assistance-listing/pipes/action-history-label.pipe.spec.ts', () => {
  ...
  it('ActionHistoryLabelPipe: transforms "retract" to "Retracted"', () => {
    expect(pipe.transform('retract')).toBe('Retracted');
  });
  it('ActionHistoryLabelPipe: transforms "publish" to "Revision Published"', () => {
    expect(pipe.transform('publish')).toBe('Revision Published');
  });
  it('ActionHistoryLabelPipe: transforms "reject_program" to "Rejected"', () => {
    expect(pipe.transform('reject_program')).toBe('Rejected');
  });
  ...
  it('ActionHistoryLabelPipe: transforms "archive" to "Archived"', () => {
    expect(pipe.transform('archive')).toBe('Archived');
  });
