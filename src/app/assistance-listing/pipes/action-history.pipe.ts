...
export class ActionHistoryPipe implements PipeTransform {
  ...
  private processHistoryItems(organizationNamesAPI: Observable<any>, actionHistoryArray: any){
    let processHistoryItem = (historyItem, index, array) => {
      ...
      //skip action_type send_omb only
      if(
        (
          (historyItem.action_type == 'send_omb' && historyItem.request_type == 'submit') && (
            (
              (array.length-1 > index && array[index+1] != null && array[index+1]['request_type'] != 'submit') && 
              (
                (array[index-1] != null && array[index-1]['request_type'] != 'submit') ||
  //              (index == 0 && array[index+1] != null && array[index+1]['request_type'] != 'submit') //submit after revise
                (index == 0 && array[index+1] != null && (array[index+1]['request_type'] != 'submit' || (array[index+1]['request_type'] == 'submit' || array[index+1]['action_type'] != 'send_omb'))) //submit after revise
              )
            ) ||
            (array.length-1 > index && array[index+1] != null && index == 0 && array[index+1]['request_type'] == 'submit' && historyItem.action_date > array[index+1]['action_date']) || //submit after reject (reverse order 0 is the lastes)
            (array.length == 1) //initial submit
          )
        ) ||
        (historyItem.action_type != 'send_omb')) {
      
        //Requestion Action Object creation
        if(historyItem.action_type != 'send_omb') {
    ...
    organizationNamesAPI.subscribe(api => {
      let objectToReturn;
      let arrayToReturn:any[] = _.flatten(actionHistoryArray._embedded.jSONObjectList.map(processHistoryItem));
      let result:any[] = [];
      for (let i in arrayToReturn){
        if (!(_.isEmpty(arrayToReturn[i]) || ((arrayToReturn[i]['title'] == null || arrayToReturn[i]['title'] == '') &&
              arrayToReturn[i]['date'] == null &&
              (arrayToReturn[i]['description'] == null || arrayToReturn[i]['description'] == '') &&
              arrayToReturn[i]['submitter'] == null))) {
          result.push(arrayToReturn[i]);
        }
      }
      objectToReturn = {
        array: result,
