...
export class ViewChangesPipe implements PipeTransform {
  transform(previousOpportunity: any, currentOpportunity:any, dictionaries: any, currentOpportunityLocation:any, previousOpportunityLocation:any): any {
    ...
    if (previousOpportunity.data && previousOpportunity.data.solicitation && previousOpportunity.data.solicitation.deadlines && previousOpportunity.data.solicitation.deadlines.response){
      previousUpdateResponseDate = currentOpportunity.data.solicitation.deadlines.response;
    } else {
    ...
    if (currentSpecialLegislation != previousSpecialLegislation && previousSpecialLegislation == null){
      specialLegislation = "New Data".italics();
      changesExist = true;
    } else if (currentSpecialLegislation != previousSpecialLegislation && previousSpecialLegislation != null){
    ...
    if (currentUpdateSetAside != previousUpdateSetAside && previousUpdateSetAside == null){
      ...
      updateSetAside = filterMultiArrayObjectPipe.transform([previousUpdateSetAside], dictionaries, 'elementId', false, '');
      updateSetAside = updateSetAside.strike();
      changesExist = true;
    }
    ...
    if (currentClassificationCode != previousClassificationCode && previousClassificationCode == null){
      ...
      classificationCode = filterMultiArrayObjectPipe.transform([previousClassificationCode], dictionaries, 'elementId', false, '').toString().strike();
      changesExist = true;
    }
    ...
    if (currentNaicsCode != previousNaicsCode && previousNaicsCode == null){
      ...
      naicsCode = filterMultiArrayObjectPipe.transform([previousNaicsCode], dictionaries, 'elementId', false, '').toString().strike();
      changesExist = true;
    }
