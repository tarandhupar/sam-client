...
@Directive({
  ...
  providers: [
    {provide: AutocompleteService, useExisting: CityServiceImpl}
  ]
})
export class SamCityServiceAutoDirective {
  ...
  constructor(private service: CityServiceImpl) { }

  ngOnChanges(event) {
    if (event.countryCode) {
      this.service.countryCode = event.countryCode.currentValue;
    }
    if (event.stateId) {
      this.service.stateId = event.stateId.currentValue;
    }
  }
}
