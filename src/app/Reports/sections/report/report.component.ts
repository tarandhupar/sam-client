...
export class ReportComponent implements OnInit {
  ...
  checkSession(cb: () => void) {
    ...
    this.api.iam.user.get(function(user) {
      ...
      // vm.url = vm.sanitizer.bypassSecurityTrustResourceUrl("http://54.197.196.14:8080/MicroStrategy/servlet/
      // mstrWeb?server=52.203.16.25&project=SAM+Prototype&evt=4001&uid=
      // "+vm.user._id+"&pwd="+vm.pwd+"&reportID="+vm.id +"&hiddensections=path,dockLeft,footer");
      vm.url = vm.sanitizer.bypassSecurityTrustResourceUrl
      ('https://csp-microstrategy.sam.gov/MicroStrategy/servlet/mstrWeb?server=10.11.34.63&project=' +
        'SAM+Prototype&evt=4001&uid=' + vm.user._id + '&pwd=' + vm.pwd + '&reportID=' + vm.id + '&hiddensections=' +
        'path,dockLeft,footerfooter&role=' + vm.user.gsaRAC[0].role);
