...
export class GrantOrEditAccess {
  ...
  comment = {
    ...
    text:
      'Lorem Ipsum Dolor Money Lorem Ipsum Dolor Money Lorem Ipsum Dolor Money Lorem Ipsum Dolor Money Lorem Ipsum Dolor Money Lorem Ipsum Dolor Money Lorem Ipsum Dolor Money Lorem Ipsum Dolor Money ',
    date: moment()
      .subtract('minutes', 5)
      .format()
  };
  ...
  ngOnInit() {
    ...
    if (this.isGov) {
      defaultOrg =
        this.route.snapshot.queryParams['org'] ||
        this.userService.getUser().departmentID ||
        '';
    } else {
    ...
    if (!this.isGov && this.route.snapshot.queryParams['org'])
      this.getEntityData(defaultOrg);
  ...
  setBreadCrumbsForRespondToRequest() {
    ...
    this.crumbs = [
      ...
      { breadcrumb: this.userFullName(), url: '../access' },
      {
        ...
        url: `/role-management/requests/${this.requestId}`
      },
        ...
        a => {
          try {
            ...
            this.roleOptions = a.grantRoles.map(r => {
              this.domainOptionsByRole[r.id] = r.supportedDomains.map(d => ({
                value: d.val,
                key: '' + d.id
              }));
  ...
  onDomainChange(domains) {
    ...
    let o = this.isEdit()
      ? this.form.value.org.map(o => '' + o.orgKey).join(',')
      : '';
    ...
    this.userAccessService
      .getDomainDefinition(
        ...
        r,
        o,
        this.isEdit() ? this.user.email : undefined
      )
      .subscribe(
        res => {
          try {
            this.domainTabs = res.map(domainAndFunctions => {
              return {
                id: domainAndFunctions.domain.id,
                name: domainAndFunctions.domain.val,
                functions: domainAndFunctions.roleDefinitionMapContent[0].functionContent
                  .filter(f => f)
                  .map(functionAndPermissions => {
                    return {
                      id: functionAndPermissions.function.id,
                      name: functionAndPermissions.function.val,
                      permissions: functionAndPermissions.permission
                        .filter(p => p)
                        .map(perm => {
                          return {
                            id: perm.id,
        ...
        err => {
          this.alertFooter.registerFooterAlert({
            ...
            title: 'Error',
            description:
              'Unable to fetch permission information for logged in user.',
            timer: 3200
          });
        }
      );
  ...
  getEntityData(orgs) {
    let entityObservable = orgs.map(e => {
      return this.userAccessService.getEntityById(e);
    });
        ...
        functionContent:
          ...
          d.functions.map(f => {
            return {
              ...
              permission:
                f.permissions &&
                f.permissions.filter(p => !p.checked).map(p => +p.id)
            };
          })
      };
    });
  ...
  onSubmit() {
    ...
    let orgs =
      !val.org || !val.org.length
        ? []
        : val.org.map(o => (this.isGov ? '' + o.orgKey : '' + o.cageCode));
    let orgMap =
      !val.org || !val.org.length
        ? []
        : val.org.map(o => ({
            orgKey: this.isGov ? o.orgKey : o.cageCode,
            name: o.name,
            level: o.level,
            fullParentPath: o.fullParentPath,
            type: o.type
          }));
    if (!this.isEditEntity && !this.isDefaultEntityPrepopulate && !this.isGov) {
      orgs =
        !this.entitySelections || !this.entitySelections.length
          ? []
          : this.entitySelections.map(o => o.cageCode);
      orgMap =
        !this.entitySelections || !this.entitySelections.length
          ? []
          : this.entitySelections.map(o => ({
              name: o.name,
              orgKey: o.cageCode
            }));
    }
    ...
    this.userAccessService[apiMethod](body, qp).subscribe(
      ...
      err => {
        ...
        if (err && err.status === 409) {
          ...
          console.error(err);
          let e: string =
            'The user already has access for this domain at one or more of the selected organization(s)';
          this.errorMessage = e;
        } else {
          console.error(err);
          this.alertFooter.registerFooterAlert({
            description:
              'Unable to complete the requested action at this time. Please try again later.',
            type: 'error',
            timer: 3200
          });
        }
